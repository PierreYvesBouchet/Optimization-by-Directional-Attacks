...
def altered_line_search_step(x, d, r_mult_list=[3/2, 1, 2/3], add_opposite=False):
    return iter(line_search_trial_directions(random_rotation(d, theta_max=0*np.pi/8), r_mult_list=r, add_opposite=add_opposite) for r in r_mult_list)
