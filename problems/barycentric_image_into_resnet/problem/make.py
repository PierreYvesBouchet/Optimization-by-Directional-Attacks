...
for f in os.listdir(path_dataset):
    if os.path.isfile(os.path.join(path_dataset, f)) and not(f == "classes.txt") and not(".md" in f):
...
path_save = "/".join([path_root, "f_bounds.pt"]);   torch.save((f_min, f_max), path_save)
