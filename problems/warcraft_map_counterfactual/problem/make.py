...
# VAE import
vae = Vae(n)
path_vae_weights = "/".join([path_build_data, "VAE_64_epoch_35.pth"])
...
path_cnn_weights = "/".join([path_build_data, "CNN_MSE_epoch_100.pth"])
...
# Projection matrix onto active subspace of f
# proj_act_sub_f = torch.eye(m)
...
def c_y(y, p_ora, p_ref, eps:float=0.5, m:int=64):
...
class Phi_model(nn.Module):
    def __init__(self):
        ...
        # self.proj_act_sub_f = proj_act_sub_f
...
class Phi_tilde_model(nn.Module):
    ...
    def __init__(self, Phi):
        ...
        # self.proj_act_sub_f = self.Phi.proj_act_sub_f
        self.relu = nn.ReLU()
        self.nb_forward_calls: int = 0
